...
var defaultConfig = {
    ...
    noTransformProperties: '' // should not be transformed properties, a string split with comma (default: ``)
};

var pxRegExp = /\b(\d+(\.\d+)?)px\b/;

// should not transform specified properties
var shouldNotTransform = function(property){
    var properties = this.config.noTransformProperties || '';
    if(properties === '') {
        return false;
    }else{
...
function Px2rem(options) {
    this.config = {};
    if(options.noTransformProperties){
        options.noTransformProperties = options.noTransformProperties.split(';');
    }
    extend(this.config, defaultConfig, options);
}

Px2rem.prototype.shouldNotTransform = shouldNotTransform;
        ...
        for (var i = 0; i < rules.length; i++) {
            ...
            for (var j = 0; j < declarations.length; j++) {
                ...
                if (declaration.type === 'declaration' && pxRegExp.test(declaration.value)) {
                    ...
                    // should not transform specified properties
                    if(self.shouldNotTransform(declaration.property)){
                        continue;
                    }
